...
def patches_by_messageid(messageid):
    ...
    patches = (
        ...
        .filter(mailthread__messageid=messageid)
        .order_by(
            "created",
        )
        .all()
    ...
    return (
        Patch.objects.select_related()
        .filter(mailthread__messageid=first_email["msgid"])
        .order_by(
            "created",
        )
        .all()
    ...
    if poc.is_closed:
        ...
        if poc != lastcf:
            messages.add_message(
                request,
                messages.INFO,
                "The status of this patch cannot be changed in this commitfest. You must modify it in the one where it's open!",
            )
            return HttpResponseRedirect("..")
...
def cfbot_ingest(message):
    ...
    created = datetime.fromisoformat(branch_status["created"]).replace(
        tzinfo=timezone.utc
    )
    ...
    old_branch = (
        CfbotBranch.objects.select_for_update().filter(patch_id=patch_id).first()
    )
    if (
        old_branch
        and old_branch.branch_id != branch_id
        and old_branch.created.replace(tzinfo=timezone.utc) > created
    ):
    ...
    if "task_status" in message:
        ...
        cursor.execute(
            """INSERT INTO commitfest_cfbottask (id, task_name, patch_id, branch_id,
                                               position, status,
                                               created, modified)
                       VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
